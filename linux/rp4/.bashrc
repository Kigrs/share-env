...
function cdh () {
    ...
    replace="s/$HOME_ESC/~/"
    restore="s/~/$HOME_ESC/"
    target=$(tac ~/.pwd_history | awk '!a[$0]++' | grep -v "^$PWD$" | sed $replace | peco --select-1 | sed $restore)
    [ -n "$target" ] && cd "$target" && pwd
    return 0
}
...
function cdp () {
    ...
    while :; do
        ...
        target=$(ls -F | grep / | sed -e "1i$files" -e "1i$(line =)" | peco --prompt ">$(pwd)/" --initial-index 2)
...
function rp4-stream () {
    ...
    #aspect="680x480"; fps="30"; jpeg_quality="30"; port=$(cat ~/mjpg-streamer/port); auth=$(cat ~/mjpg-streamer/basic_auth);
    aspect="1280x720"; fps="10"; jpeg_quality="30"; port=$(cat ~/mjpg-streamer/port); auth=$(cat ~/mjpg-streamer/basic_auth);
