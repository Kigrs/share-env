...
home_dir=$HOME
share_env_dir=$HOME/share-env/$OS_NAME
SRCH_DPTH=5

echo -e "# Make symlink by following command."

echo -e "\n- New files -"
for files in $(basename $(find $share_env_dir -name "\.*" -mindepth 1 -maxdepth 1 -not -iwholename '*/.git*' -not -iwholename '*/.DS_Store')) ; do
...
for files in $(basename $(find $share_env_dir -name "\.*" -mindepth 1 -maxdepth 1 -not -iwholename '*/.git*' -not -iwholename '*/.DS_Store')) ; do
    ...
    [ -n "$linkfile" ] &&  ls -lG $linkfile #&& echo -e "\033[1mln -si $share_env_dir/$files $linkfile\033[m"
done
